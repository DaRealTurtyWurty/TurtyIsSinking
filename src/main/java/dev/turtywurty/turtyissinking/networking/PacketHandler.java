...
public final class PacketHandler {
    ...
    public static final SimpleChannel INSTANCE = NetworkRegistry.newSimpleChannel(
        new ResourceLocation(TurtyIsSinking.MODID, "main"), () -> PROTOCOL_VERSION, PROTOCOL_VERSION::equals,
        PROTOCOL_VERSION::equals);
    
    public static void init() {
        int index = 0;
        INSTANCE.messageBuilder(SOpenBackpackPacket.class, index++, NetworkDirection.PLAY_TO_SERVER)
            .encoder(SOpenBackpackPacket::encode).decoder(SOpenBackpackPacket::new)
            .consumerMainThread(SOpenBackpackPacket::handle).add();
        INSTANCE.messageBuilder(SSawBonePacket.class, index++, NetworkDirection.PLAY_TO_SERVER)
            .encoder(SSawBonePacket::encode).decoder(SSawBonePacket::new).consumerMainThread(SSawBonePacket::handle)
            .add();
        INSTANCE.messageBuilder(CSyncPlayerBonesPacket.class, index++, NetworkDirection.PLAY_TO_CLIENT)
            .encoder(CSyncPlayerBonesPacket::encode).decoder(CSyncPlayerBonesPacket::new)
            .consumerMainThread(CSyncPlayerBonesPacket::handle).add();
